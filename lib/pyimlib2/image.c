...
PyObject *Image_PyObject__get_raw_data(PyObject *self_object, PyObject *args)
{
    ...
    if (self->raw_data) {
        return Py_BuildValue("O", PyBuffer_FromMemory(self->raw_data, self->raw_data_size));
    }

    if (!PyArg_ParseTuple(args, "|s", &format))
      return PyErr_SetString(PyExc_AttributeError, ""), \
        (PyObject*)NULL;
    ...
    return Py_BuildValue("O", PyBuffer_FromMemory(buffer, self->raw_data_size));
}
...
PyObject *Image_PyObject__to_sdl_surface(PyObject *self, PyObject *args)
{
    ...
    return Py_None;
#else
    return PyErr_SetString(PyExc_ValueError, "pygame support missing"), 
        (PyObject*)NULL;
#endif
}
