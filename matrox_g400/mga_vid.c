...
static int
mga_vid_set_config (mga_vid_config_t * config)
{
  int x, y, sw, sh, dw, dh;
  ...
  if (config->format == MGA_VID_FORMAT_YV12
    ...
    if (config->format == MGA_VID_FORMAT_YV12) {
      regs.besa1corg =
	(uint32_t) mga_src_base + baseadrofs + regs.bespitch * sh;
      regs.besa2corg =
	(uint32_t) mga_src_base + baseadrofs + 1 * frame_size +
	regs.bespitch * sh;
      regs.besb1corg =
	(uint32_t) mga_src_base + baseadrofs + 2 * frame_size +
	regs.bespitch * sh;
      regs.besb2corg =
	(uint32_t) mga_src_base + baseadrofs + 3 * frame_size +
	regs.bespitch * sh;
  ...
  regs.besv2srclst = regs.besv1srclst =
    sh - 1 - (((ofstop * regs.besviscal) >> 16) & 0x03FF);
...
void
mga_handle_irq (int irq, void *dev_id, struct pt_regs *pregs)
{
//      static int frame=0;
...
static int
mga_vid_ioctl (struct inode *inode, struct file *file, unsigned int cmd,
	       unsigned long arg)
{
  int frame;
  mga_vid_geomconfig_t geomconf;


  switch (cmd) {
  case MGA_VID_GEOMCONFIG:

    if (copy_from_user
	(&geomconf, (mga_vid_geomconfig_t *) arg, sizeof (mga_vid_geomconfig_t))) {
      printk (KERN_ERR "mga_vid: failed geomconf copy from userspace\n");
      return (-EFAULT);
    }

    printk (KERN_DEBUG
            "mga_vid: Geomconf new dest: %dx%d+%d+%d (old: %dx%d+%d+%d) "
            "(src: %dx%d)\n",
            geomconf.dest_width, geomconf.dest_height,
            geomconf.x_org, geomconf.y_org,
            mga_config.dest_width, mga_config.dest_height,
            mga_config.x_org, mga_config.y_org,
            mga_config.src_width, mga_config.src_height);

    mga_config.x_org = geomconf.x_org;
    mga_config.y_org = geomconf.y_org;
    mga_config.dest_width = geomconf.dest_width;
    mga_config.dest_height = geomconf.dest_height;
  ...
  case MGA_VID_CONFIG:
    ...
    if (copy_from_user
	(&mga_config, (mga_vid_config_t *) arg, sizeof (mga_vid_config_t))) {
    ...
    if (mga_config.frame_size > 1024 * 768 * 2) {
    ...
    mga_src_base =
      (mga_ram_size * 0x100000 -
       mga_config.num_frames * mga_config.frame_size);
    if (mga_src_base < 0) {
    ...
    if (copy_to_user
	((mga_vid_config_t *) arg, &mga_config, sizeof (mga_vid_config_t))) {
...
static int
mga_vid_find_card (void)
{
  struct pci_dev *dev = NULL;
  unsigned int card_option;

  if ((dev =
       pci_find_device (PCI_VENDOR_ID_MATROX, PCI_DEVICE_ID_MATROX_G550,
			NULL))) {
    is_g400 = 1;
    printk (KERN_INFO "mga_vid: Found MGA G550\n");
  } else
    if ((dev =
	 pci_find_device (PCI_VENDOR_ID_MATROX, PCI_DEVICE_ID_MATROX_G400,
			  NULL))) {
    is_g400 = 1;
    printk (KERN_INFO "mga_vid: Found MGA G400/G450\n");
  } else
    if ((dev =
	 pci_find_device (PCI_VENDOR_ID_MATROX, PCI_DEVICE_ID_MATROX_G200_AGP,
			  NULL))) {
    is_g400 = 0;
    printk (KERN_INFO "mga_vid: Found MGA G200 AGP\n");
  } else
    if ((dev =
	 pci_find_device (PCI_VENDOR_ID_MATROX, PCI_DEVICE_ID_MATROX_G200_PCI,
			  NULL))) {
    is_g400 = 0;
...
#else
  mga_mmio_base =
    ioremap_nocache (dev->base_address[1] & PCI_BASE_ADDRESS_MEM_MASK, 0x4000);
...
//          printk("List resources -----------\n");
	...
	printk (KERN_DEBUG
		...
		res->start, res->end, size, res->flags);
	if (res->flags & (IORESOURCE_MEM | IORESOURCE_PREFETCH)) {
	  if (size > mga_ram_size && size <= 64)
	    mga_ram_size = size;
	}
      }
    }
  ...
  if (mga_irq != -1) {
    int tmp =
      request_irq (mga_irq, mga_handle_irq, SA_INTERRUPT | SA_SHIRQ,
...
static ssize_t
mga_vid_read (struct file *file, char *buf, size_t count, loff_t * ppos)
{
  return -EINVAL;
}

static ssize_t
mga_vid_write (struct file *file, const char *buf, size_t count, loff_t * ppos)
{
  return -EINVAL;
}

static int
mga_vid_mmap (struct file *file, struct vm_area_struct *vma)
{
...
static int
mga_vid_release (struct inode *inode, struct file *file)
{
#if 0
...
static long long
mga_vid_lseek (struct file *file, long long offset, int origin)
{
  return -ESPIPE;
}

static int
mga_vid_open (struct inode *inode, struct file *file)
{
...
static int
mga_vid_initialize (void)
{
  mga_vid_in_use = 0;

  printk (KERN_INFO
	  "Matrox MGA G200/G400/G450 YUV Video interface v2.02 (c) Aaron Holtzman, A'rpi & Krister Lagerström\n");
